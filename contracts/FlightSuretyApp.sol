...
// OpenZeppelin's SafeMath library, when used correctly, protects agains such
// bugs
...
contract FlightSuretyData {
  ...
  function registerFlight(address airline, string flight,
              uint256 timestamp) external;
  function buy(address airline, string flight, uint256 timestamp,
              address passanger) payable external;
  function creditInsurees(address airline, string flight,
              uint256 timestamp) external;
  function clearInsurees(address airline, string flight,
              uint256 timestamp) external;
...
event RegisterAirline(address airline, address requestedBy);
  ...
  */
  ...
  function passangerCredit() view external returns(uint256 credit)
  {
    return flightSuretyData.passangerCredit(msg.sender);
  }
...
// region ORACLE MANAGEMENT
  ...
  event FlightStatusInfo(address airline, string flight, uint256 timestamp,
    uint8 status);
  bool toggle = false;
  event OracleReport(address airline, string flight, uint256 timestamp,
    uint8 status, address ria, bool rio);
  ...
  // For the response to be accepted, there must be a pending request that is
  // open and matches one of the three Indexes randomly assigned to the oracle
  // at the time of registration (i.e. uninvited oracles are not welcome)
    ...
    uint256 timestamp, uint8 statusCode ) external
  {
    ...
    emit OracleReport(airline, flight, timestamp, statusCode, ri.requester,
      ri.isOpen);
